...
#        self.layer_norm_1 = nn.LayerNorm(configs.pssm_dim + configs.seq_dim + configs.netsurf_dim, elementwise_affine=False)
#        self.layer_norm_2 = nn.LayerNorm(configs.pssm_dim + configs.seq_dim + configs.netsurf_dim, elementwise_affine=False)
#        self.layer_norm_3 = nn.LayerNorm(configs.pssm_dim + configs.seq_dim + configs.netsurf_dim, elementwise_affine=False)
...
#        atten_s = self.layer_norm_1(s)
#        atten_target_s = self.layer_norm_2(target_s)
#        atten_features = self.layer_norm_3(features)
        
        y = t.cat((self.linear1(t.mul(s, features)), self.linear2(t.mul(target_s, features))), dim=-1) 
#        y = t.cat((t.mul(s, features), t.mul(target_s, features)), dim=-1)
#        y = t.cat((self.linear1(y), msa_features.unsqueeze(-1)), dim=-1)
        y = t.cat((y, msa_features.unsqueeze(-1)), dim=-1)
        y = self.LeakyRelu(y)
        x = y * self.q1
        x = x.sum(dim=-1)
        x = self.layer_norm(x)
        temp_for_7cei_A = self.softmax(x).unsqueeze(-1)
#        if temp_for_7cei_A.shape[0] == 112:
        cur_moment = time.time()
#        t.save(temp_for_7cei_A, 'attention_' + str(cur_moment) + '.pt')
